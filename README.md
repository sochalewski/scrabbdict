...
- alphabet table: the distinct `UInt16` Unicode scalar values used by edge labels, stored in strictly ascending order,
...
- the alphabet table contains distinct `UInt16` Unicode scalars in strictly ascending order,
- each node's edge block is stored by strictly ascending alphabet index.

Both word-search methods traverse the stored edges depth-first and emit a word before visiting its descendants. Consequently, `DAWG.words(from:minLength:)` and `DAWG.words(matching:)` return words in ascending `String.<` order for the normalized dictionary entries used by the project. Consuming unavailable rack letters or following fixed pattern positions only skips branches, so it does not change the relative order of the remaining results.

Callers may rely on this contract. A stable sort by score alone preserves the DAWG's scalar-lexicographic order between words with equal scores. This is sufficient for the normalized English and French dictionaries, whose alphabets contain only ASCII letters. Polish user-facing results apply Polish alphabetical order to score ties because Unicode scalar order differs from Polish collation. Tests should compare DAWG result arrays directly rather than sorting them first.
